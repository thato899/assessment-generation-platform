...
M2 Vertical Projectile Motion is implemented, M3 Momentum & Impulse is complete, and M4 Newton's Laws has completed implementation and final verification. The public v1 API supports the approved deterministic Physical Sciences routes for vertical projectile motion, Momentum & Impulse, and Grade 11 Newton's Laws. M5 Work, Energy & Power has its authoritative solver and deterministic generation merged; Issue #77 is the active authored-only technical SVG renderer. Mathematics remains later in the roadmap. See [STATUS.md](STATUS.md) and [PLANNING.md](PLANNING.md) for the current milestone details.
